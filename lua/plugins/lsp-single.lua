-- Alternative Intelephense-only PHP LSP configuration
-- Use this if PhpActor is still causing performance issues
	...
	config = function()
		...
		require("mason-lspconfig").setup({
			ensure_installed = {
				...
				"ts_ls",
				"intelephense",
				-- Remove phpactor for single LSP setup
				"rust_analyzer",
			},
		...
		require("lspsaga").setup({
			-- Minimal Lspsaga setup for better performance
			code_action = {
				num_shortcut = true,
		...
		local on_attach = function(client, bufnr)
			...
			-- PHP-specific commands for class resolution
		...
		local servers = {
			...
			ts_ls = {},
			pyright = {},
			-- Enhanced Intelephense configuration (single LSP for better performance)
			intelephense = {
				settings = {
					intelephense = {
						files = {
						...
						environment = {
							includePaths = {},
							phpVersion = "7.4.0", -- Adjust to match your project
						},
						diagnostics = {
							enable = true,
							run = "onSave", -- Better performance than "onType"
							...
							undefinedVariables = true, -- Enable to catch undefined class errors
							unusedSymbols = true,
							typeErrors = true,
						},
						format = {
						...
						completion = {
							...
							maxItems = 100, -- Increased for better completion
						},
						phpdoc = {
							returnVoid = true,
							textFormat = "snippet",
